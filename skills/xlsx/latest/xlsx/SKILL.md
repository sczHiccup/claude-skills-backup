---
name: xlsx
description: "全面的电子表格创建、编辑和分析，支持公式、格式化、数据分析和可视化。当 Claude 需要处理电子表格 (.xlsx, .xlsm, .csv, .tsv 等) 时： (1) 创建带有公式和格式的新电子表格， (2) 读取或分析数据， (3) 修改现有电子表格并保留公式， (4) 在电子表格中进行数据分析和可视化，或 (5) 重新计算公式"
...
### 零公式错误
- 每个 Excel 模型必须交付时没有任何公式错误 (#REF!, #DIV/0!, #VALUE!, #N/A, #NAME?)

### 保留现有模板（更新模板时）
- 修改文件时，研究并准确匹配现有格式、样式和约定
- 不要对已有模式的文件强加标准化格式
- 现有模板约定始终优先于这些指南
...
#### 行业标准颜色约定
- **蓝色文本 (RGB: 0,0,255)**: 硬编码输入，以及用户将更改的场景数字
...
#### 硬编码文档要求
- 在单元格旁边注释或说明（如果在表格末尾）。格式：“来源：[系统/文档]，[日期]，[具体参考]，[URL（如适用）]”
- 示例：
  - “来源：公司 10-K，FY2024，第 45 页，收入说明，[SEC EDGAR URL]”
  - “来源：公司 10-Q，2025 年第二季度，附录 99.1，[SEC EDGAR URL]”
  - “来源：彭博终端，2025 年 8 月 15 日，AAPL US Equity”
  - “来源：FactSet，2025 年 8 月 20 日，Consensus Estimates Screen”
...
**需要 LibreOffice 进行公式重新计算**：您可以假设已安装 LibreOffice，以使用 `recalc.py` 脚本重新计算公式值。该脚本在首次运行时会自动配置 LibreOffice。
...
使用 openpyxl 创建或修改的 Excel 文件包含公式作为字符串，但不包含计算值。使用提供的 `recalc.py` 脚本重新计算公式：
...
### 基本验证
- [ ] **测试 2-3 个示例引用**：在构建完整模型之前验证它们是否提取正确值
- [ ] **列映射**：确认 Excel 列匹配（例如，第 64 列 = BL，而不是 BK）
- [ ] **行偏移**：记住 Excel 行是 1 索引（DataFrame 行 5 = Excel 行 6）

### 常见陷阱
- [ ] **NaN 处理**：使用 `pd.notna()` 检查空值
- [ ] **最右侧列**：FY 数据通常在第 50 列及以上
- [ ] **多个匹配**：搜索所有出现，而不仅仅是第一个
- [ ] **除以零**：在公式中使用 `/` 之前检查分母 (#DIV/0!)
- [ ] **错误引用**：验证所有单元格引用指向预期单元格 (#REF!)
- [ ] **跨工作表引用**：使用正确格式 (Sheet1!A1) 连接工作表

### 公式测试策略
- [ ] **从小开始**：在 2-3 个单元格上测试公式，然后再广泛应用
...
### 库选择
- **pandas**：最佳用于数据分析、大批量操作和简单数据导出
- **openpyxl**：最佳用于复杂格式、公式和 Excel 特定功能

### 使用 openpyxl
- 单元格索引为 1 基（row=1, column=1 指的是单元格 A1）
